(function (stringArrayFunction, comparisonValue) {
  var stringArray = stringArrayFunction();
  while (true) {
    try {
      var expression = -parseInt("582611YGlboA") / 1 * (parseInt("2JATjvB") / 2) + -parseInt("551904yvMTOE") / 3 + -parseInt("1228740FXPVKd") / 4 + parseInt("53985BANyss") / 5 * (-parseInt("78bymMhj") / 6) + parseInt("838271BBWBWT") / 7 * (parseInt("16thAptW") / 8) + parseInt("32733ZfdivB") / 9 * (-parseInt("1210dtlMQV") / 10) + parseInt("121dutkZN") / 11 * (parseInt("1891116VRGwWy") / 12);
      if (expression === comparisonValue) {
        break;
      } else {
        stringArray.push(stringArray.shift());
      }
    } catch (e) {
      stringArray.push(stringArray.shift());
    }
  }
})(__STRING_ARRAY__, 318827);
var input = require("fs")["readFileSync"]("/dev/stdin", "utf8");
var arr = input["trim"]()["split"]("\n");
var n = arr["shift"]() - 0;
function __DECODE_0__(vXvuRj, key) {
  var stringArray = __STRING_ARRAY__();
  __DECODE_0__ = function (index, key) {
    index = index - 236;
    var value = stringArray[index];
    return value;
  };
  return __DECODE_0__(vXvuRj, key);
}
var a = arr.shift().split(" ").map(Number);
var b = a.slice()["sort"]((x, y) => x - y);
var q = arr["shift"]() - 0;
function __STRING_ARRAY__() {
  var _0x2633ab = ["121dutkZN", "78bymMhj", "838271BBWBWT", "1891116VRGwWy", "shift", "53985BANyss", "1210dtlMQV", "2JATjvB", "582611YGlboA", "551904yvMTOE", "map", "readFileSync", "log", "32733ZfdivB", "trim", "/dev/stdin", "keys", "sort", "1228740FXPVKd", "utf8", "length", "unshift", "split", "16thAptW"];
  __STRING_ARRAY__ = function () {
    return _0x2633ab;
  };
  return __STRING_ARRAY__();
}
var xy = arr["map"](v => v["split"](" ")["map"](Number).map(v => v - 1));
xy["unshift"]([0, 0]);
var A = {};
var B = {};
for (var i = 0; i < a["length"]; i++) {
  if (a[i] != b[i]) {
    A[i] = a[i];
    B[i] = b[i];
  }
}
var BB = "";
for (var k in B) {
  BB += B[k];
}
var flag = false;
for (var i = 0; i <= q; i++) {
  [A[xy[i][0]], A[xy[i][1]]] = [A[xy[i][1]], A[xy[i][0]]];
  var AA = "";
  for (var k in A) {
    AA += A[k];
  }
  if (AA == BB) {
    flag = true;
    break;
  }
}
if (Object["keys"](A)["length"] == 0) {
  console["log"](0);
} else if (flag) {
  console["log"](i);
} else {
  console["log"](-1);
}
